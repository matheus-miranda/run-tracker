...
object LocationDataCalculator {
    ...
    fun getMaxSpeedKmh(locations: List<List<LocationTimestamp>>): Double {
        return locations.maxOf { locationSet ->
            locationSet.zipWithNext { locations1, location2 ->
                val distance = locations1.location.location.distanceTo(other = location2.location.location)
                val hoursDifference = (location2.durationTimestamp - locations1.durationTimestamp)
                    .toDouble(DurationUnit.HOURS)

                if (hoursDifference == 0.0) {
                    0.0
                } else {
                    (distance / 100) / hoursDifference
                }
            }.maxOrNull() ?: 0.0
        }
    }
