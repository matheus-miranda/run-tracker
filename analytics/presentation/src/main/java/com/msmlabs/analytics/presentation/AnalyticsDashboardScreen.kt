...
package com.msmlabs.analytics.presentation
...
import com.msmlabs.analytics.presentation.components.AnalyticsCard
...
fun AnalyticsDashboardScreenRoot(
    ...
    AnalyticsDashboardScreen(
        ...
        onAction = { action ->
            when (action) {
                AnalyticsAction.OnBackClick -> onBackClick.invoke()
            }
        }
    )
}
    ...
    RuntrackerScaffold(
        topAppBar = {
            RuntrackerToolbar(
                ...
                onBackClick = { onAction.invoke(AnalyticsAction.OnBackClick) }
            )
        },
        content = { padding ->
            if (state == null) {
    ...
    RunTrackerTheme {
        AnalyticsDashboardScreen(
            state = AnalyticsDashboardState(
                ...
                totalTimeRun = "0d 0h 15m",
                fastestEverRun = "35 km/h",
                avgDistance = "0.1 km",
                avgPace = "07:10"
            ),
            onAction = {}
